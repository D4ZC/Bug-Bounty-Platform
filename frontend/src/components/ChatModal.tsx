...
const mockMessages = {
  1: [
    { from: 'Ana', text: '¡Hola equipo!', time: '10:00' },
    { from: 'Yo', text: '¡Hola Ana!', time: '10:01' },
  ],
  2: [
    { from: 'Luis', text: '¿Listos para el reto?', time: '10:02' },
    { from: 'Yo', text: '¡Sí!', time: '10:03' },
  ],
  3: [
    { from: 'Marta', text: '¿Quién va a reportar?', time: '10:04' },
    { from: 'Yo', text: 'Puedo hacerlo yo.', time: '10:05' },
  ],
};
  ...
  return (
    <div className="fixed top-[50px] left-0 z-50 w-1/2 h-[100vh] bg-white rounded-r-2xl shadow-2xl flex flex-col animate-fade-in border border-gray-300">
      ...
      <div className="flex flex-row gap-2 px-6 py-2 border-b border-gray-100 bg-white">
        <button
          className={`px-4 py-2 rounded-t-lg font-bold ${tab === 'global' ? 'bg-yellow-300 text-black' : 'bg-gray-100 text-gray-700'}`}
          onClick={() => setTab('global')}
        >
          GLOBAL
        </button>
        <button
          className={`px-4 py-2 rounded-t-lg font-bold ${tab === 'equipo' ? 'bg-yellow-300 text-black' : 'bg-gray-100 text-gray-700'}`}
          onClick={() => setTab('equipo')}
        >
          EQUIPO
        </button>
        <button
          className={`px-4 py-2 rounded-t-lg font-bold ${tab === 'usuario' ? 'bg-yellow-300 text-black' : 'bg-gray-100 text-gray-700'}`}
        ...
        <div className="w-1/3 bg-gray-50 border-r border-gray-200 flex flex-col items-center justify-start p-4">
          {tab === 'equipo' ? (
            <div className="flex flex-col items-center gap-2 w-full">
              <img src={mockTeam.avatar} alt="avatar" className="w-16 h-16 rounded-full border-2 border-yellow-400 mb-2" />
            ...
            <div className="flex flex-col gap-2 w-full">
              {mockTeam.users.map(u => (
                <button
                  key={u.id}
                  className={`flex items-center gap-2 px-2 py-2 rounded-lg w-full text-left transition ${selectedUser === u.id ? 'bg-yellow-200' : 'hover:bg-gray-200'}`}
          ...
          {tab === 'global' ? (
            <div className="flex flex-col h-full">
              <div className="flex-1 overflow-y-auto mb-4 flex flex-col gap-2">
                {globalMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.from === 'Yo' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`px-3 py-2 rounded-lg max-w-xs ${msg.from === 'Yo' ? 'bg-yellow-200 text-black' : 'bg-gray-200 text-gray-800'}`}>
              ...
              <div className="flex items-center gap-2 mt-auto pb-12">
                <input
                  className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-yellow-300"
            ...
            <div className="flex flex-col h-full">
              <div className="flex-1 overflow-y-auto mb-4 flex flex-col gap-2">
                {(mockMessages[selectedUser] || []).map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.from === 'Yo' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`px-3 py-2 rounded-lg max-w-xs ${msg.from === 'Yo' ? 'bg-yellow-200 text-black' : 'bg-gray-200 text-gray-800'}`}>
                      <span className="block text-xs font-semibold mb-1">{msg.from}</span>
                      <span>{msg.text}</span>
