...
const mockPointsList = [
  { id: 1, cantidad: 100, motivo: 'Vulnerabilidad Crítica', fecha_obtencion: '2023-07-01', fecha_expiracion: '2024-07-01' },
  { id: 2, cantidad: 75, motivo: 'Vulnerabilidad Alta', fecha_obtencion: '2023-10-15', fecha_expiracion: '2024-10-15' },
  { id: 3, cantidad: 150, motivo: 'MVP', fecha_obtencion: '2024-01-10', fecha_expiracion: '2025-01-10' },
  { id: 4, cantidad: 50, motivo: 'Vulnerabilidad Media', fecha_obtencion: '2024-02-20', fecha_expiracion: '2025-02-20' },
];

// Mock de canjes realizados
const mockRedemptions = [
  { id: 1, recompensa: 'Camiseta Oficial', puntos_gastados: 200, fecha_canje: '2023-12-01' },
  { id: 2, recompensa: 'Sticker Pack', puntos_gastados: 50, fecha_canje: '2024-02-20' },
  { id: 3, recompensa: 'Skin Cyber Ninja', puntos_gastados: 400, fecha_canje: '2024-03-15' },
];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-black text-white">
      <div className="max-w-4xl mx-auto py-10 px-4">
        <header className="text-center mb-8">
          <h1 className="text-4xl font-extrabold tracking-tight drop-shadow-lg mb-4">Perfil de Usuario</h1>
          <div className="bg-blue-800 rounded-lg px-6 py-3 inline-flex items-center gap-3 shadow-lg">
            ...
            <span className="bg-yellow-600 text-white font-bold px-3 py-1 rounded-full">Nivel {mockUser.level}</span>
            <span className="bg-purple-600 text-white font-bold px-3 py-1 rounded-full">#{mockUser.rank} Global</span>
          </div>
        </header>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {mockPointsList.map((point) => (
                    <tr key={point.id} className="border-b border-blue-700 hover:bg-blue-800 hover:bg-opacity-30">
                      <td className="py-3 px-4 font-bold text-green-400">{point.cantidad}</td>
                      <td className="py-3 px-4">{point.motivo}</td>
                      <td className="py-3 px-4 text-blue-200">{point.fecha_obtencion}</td>
                      <td className="py-3 px-4 text-yellow-200">{point.fecha_expiracion}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {mockRedemptions.map((redemption) => (
                    <tr key={redemption.id} className="border-b border-purple-700 hover:bg-purple-800 hover:bg-opacity-30">
                      <td className="py-3 px-4 font-bold">{redemption.recompensa}</td>
                      <td className="py-3 px-4 text-red-400 font-bold">{redemption.puntos_gastados}</td>
                      <td className="py-3 px-4 text-purple-200">{redemption.fecha_canje}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
