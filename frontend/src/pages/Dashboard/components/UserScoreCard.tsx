...
const UserScoreCard: React.FC<{ users: User[] }> = ({ users }) => {
  ...
  return (
    <div className="relative rounded-3xl border-4 border-green-400 shadow-2xl p-8 glassmorphism flex flex-col justify-between overflow-hidden" style={{fontFamily, clipPath:'polygon(0 0, 100% 0, 98% 100%, 2% 100%)'}}>
      ...
      <h2 className="text-2xl font-extrabold text-green-300 mb-2 text-center font-mono animate-glitch-text">{t('User Score')}</h2>
      <p className="text-green-100 text-center mb-1 font-mono animate-glitch-text">{t('¡Los mejores jugadores individuales!')}</p>
      <div className="flex justify-center mb-4">
        <span className="inline-block bg-green-700/60 text-green-200 px-4 py-2 rounded-lg font-mono text-lg font-semibold animate-glitch-btn">{t('TOP 3 - Usuarios')}</span>
      </div>
      <div className="overflow-x-auto z-10 relative">
        <table className="w-full text-green-100 text-sm font-mono">
          <thead>
            <tr>
          ...
          <tbody>
            {users.map((user, i) => (
              <tr key={user.name} className="border-b border-green-700/50 last:border-none hover:bg-green-900/10 transition-all">
